...
def recorrido_profundidad(event):
    ...
    while not stack.estavacio():
        ...
        for vecino in range (vecinos.len):
            indice, peso = nodo_actual.adyacentes.getAt(vecino)  # Obtenemos índice de la arista
            nodo_vecino = grafo.obtener_iesimo_vertice(indice)
            ...
            if nodo_vecino.estado == 0:  # Solo encolamos si está en espera
                ...
                # Dibujar la arista en verde
                x1 = lista_vertices[actual].left + 25
                y1 = lista_vertices[actual].top + 25
                x2 = lista_vertices[indice].left + 25
                y2 = lista_vertices[indice].top + 25
                genera_arista([x1, y1], [x2, y2], "green")  # Resaltar en verde
